...
//        String[] param = {"2016-09-15 01:00:04.002 2.0s",
//                            "2016-09-15 01:00:07.000 2s",
//                             "2016-09-15 01:00:07.130 2s"};
    ...
    public static int solution2(String[] lines) throws Exception {
        ...
        for(int i=0; i<lines.length; i++) {
            ...
            Date parse = format.parse(time[1]);
            long curMilliSecend = parse.getTime();
            int maxCnt = 0;

            for(int j=i; j<lines.length; j++) {
                String[] nextTime = lines[j].split(" ");
                Date nextParse = format.parse(nextTime[1]);
                long nextMillSecond = nextParse.getTime();

                double duration = Double.parseDouble(nextTime[2].split("s")[0]);

                long nextStart = (long) (nextMillSecond - (duration * 1000) + 1);


                if(curMilliSecend + 1000 > nextStart) {
                    maxCnt++;
